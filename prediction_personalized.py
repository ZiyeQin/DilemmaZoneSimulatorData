...
personalized_info = pd.read_csv("personalized_info.csv")

data = pd.merge(trajectory_data, decision_data, on=['driver_id', 'trajectory_id'], how='inner')
data = pd.merge(data, personalized_info, on='driver_id', how='inner')
...
class DilemmaZoneDataset(Dataset):
    def __init__(self, data, input_length=93):
        ...
        self.personalized_info = data[['avg_yellow_time', 'avg_speed_at_decision', 'avg_distance_to_stop_line_at_decision', 'probability_of_go']].values[::input_length]
        
    def __len__(self):
        return len(self.stop_go_labels)
    
    def __getitem__(self, idx):
        distance = torch.tensor(self.distances[idx], dtype=torch.float32).permute(1, 0)
        speed = torch.tensor(self.speeds[idx], dtype=torch.float32).permute(1, 0)
        acceleration = torch.tensor(self.accelerations[idx], dtype=torch.float32).permute(1, 0)
        
        stop_go_label = torch.tensor(self.stop_go_labels[idx], dtype=torch.float32)
        decision_time = torch.tensor(self.decision_times[idx], dtype=torch.float32)
        
        personalized_info = torch.tensor(self.personalized_info[idx], dtype=torch.float32)
        
        return distance, speed, acceleration, personalized_info, stop_go_label, decision_time

class DilemmaZoneCNN(nn.Module):
    def __init__(self, input_length, personalized_info_length=4):
        ...
        self.fc1 = nn.Linear(flattened_size * 3 + personalized_info_length, 64)
        self.fc2 = nn.Linear(64, 32)
        
        self.stop_go_output = nn.Linear(32, 1)
        self.decision_time_output = nn.Linear(32, 1)
        
    def forward(self, distance, speed, acceleration, personalized_info):
        ...
        combined = torch.cat((distance, speed, acceleration, personalized_info), dim=1)
...
#     distance, speed, acceleration, personalized_info, stop_go_label, decision_time = batch
#     print("Distance:", distance)
#     print("Speed:", speed.shape)
#     print("Acceleration:", acceleration.shape)
#     print("Personalized Info:", personalized_info.shape)
#     print("Stop/Go Label:", stop_go_label.shape)
#     print("Decision Time:", decision_time.shape)
#     break  
...
for epoch in range(epochs):
    model.train()
    total_loss = 0
    for distance, speed, acceleration, personalized_info, stop_go_label, decision_time in train_loader:
        stop_go_pred, decision_time_pred = model(distance, speed, acceleration, personalized_info)
...
with torch.no_grad():
    ...
    for distance, speed, acceleration, personalized_info, stop_go_label, decision_time in test_loader:
        stop_go_pred, decision_time_pred = model(distance, speed, acceleration, personalized_info)
